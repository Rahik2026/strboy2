    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.05 }} className="group relative">
      <Link href={`/product/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] bg-ink-50 rounded-2xl overflow-hidden mb-3">
          ...
          {product.trending && <div className="absolute top-3 right-3 bg-ink-950/80 backdrop-blur text-brand-300 text-[10px] font-bold px-2.5 py-1 rounded-full">Trending</div>}
          {isOutOfStock && (
          ...
          <div className="absolute inset-x-0 bottom-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
            <div className="flex gap-2">
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleAddToCart(); }}
                ...
                className={`flex-1 flex items-center justify-center gap-2 text-xs font-semibold py-2.5 rounded-xl shadow-soft transition-colors ${isOutOfStock ? 'bg-ink-300 text-white cursor-not-allowed' : 'bg-white text-ink-900 hover:bg-brand-700 hover:text-white'}`}>
                <ShoppingBag className="w-3.5 h-3.5" /> {isOutOfStock ? "Out of Stock" : "Add to Cart"}
              </button>
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleWishlist(); }}
                className={`p-2.5 rounded-xl shadow-soft transition-colors ${inWishlist ? "bg-red-500 text-white" : "bg-white text-ink-700 hover:bg-red-50 hover:text-red-500"}`}>
